-- The payables subledger, one row per vendor invoice, converted to EUR at
-- the issue month's rate.
...
with src as (
    select
        ...
        category
    from read_csv('data/raw/ap_invoices.csv', header = true, all_varchar = true)
),
